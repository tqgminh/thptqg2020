...
public class B00Ranking {
	public List<Student> student = new ArrayList<Student>();
	
	public B00Ranking() throws NumberFormatException, IOException {
		File file = new File("data\\B00.txt");
	...
	public static int binarySearch(List<Student> student, double target) {
		...
		while(lower <= upper) {
			mid = (lower+upper)/2;
			if(student.get(mid).getB00() < target)
				upper = mid-1;
			else if(student.get(mid).getB00() > target)
				lower = mid+1;
			else
				return mid;
		}
	...
	public String rankingB00(Student std) {
		//Collections.sort(student, new A00Comparator());
		int here = binarySearch(student, std.getB00());
		System.out.println(here);
		int up = here, down = here;
		while(up >= 0) {
			if(student.get(up).getB00() == std.getB00())
				up--;
			else break;
		}
		while(down <= student.size()-1) {
			if(student.get(down).getB00() == std.getB00())
				down++;
			else break;
		}
